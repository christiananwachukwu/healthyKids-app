...
function displayVideos(videoList) {
    resultsContainer.innerHTML = "";
    const videoItems = videoList.filter(item => item.id.kind === 'youtube#video');

    if (videoItems.length === 0) {
        // Use a clearer message to distinguish between error and no results
        resultsContainer.innerHTML = "<p>No videos found for your search. The API key may be invalid. Please try another topic.</p>";
        return;
    }
...
async function handleUserSearch() {
    ...
    if (query.length < 2) {
        resultsContainer.innerHTML = "<p>Please type at least 2 characters to search.</p>";
        return;
    }
